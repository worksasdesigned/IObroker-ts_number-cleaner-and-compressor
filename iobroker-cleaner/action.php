...
function handle_analyze_datapoints(PDO $pdo): void {
	try {
		// Use a proper multi-line SQL string; avoid escaped newlines inside single quotes
		$sql = "SELECT dp.id AS id, dp.name AS name, COUNT(*) AS anzahl\n" .
		    "FROM ts_number tn\n" .
		    "JOIN datapoints dp ON tn.id = dp.id\n" .
...
function handle_test(PDO $pdo): void {
	try {
		...
		// Selective logging buffers/state
		$contextBuffer = [];
		$bufferCapacity = 10; // keep last 10 rows
		$postEmitRemaining = 0; // emit next N rows after anomaly
		$emittedTs = [];
		...
		while ($row = $stmt->fetch()) {
			...
			$shouldEmit = false;
			if ($isAnomaly) {
				// Emit up to 10 previous rows for context
				foreach ($contextBuffer as $prev) {
					$prevTs = $prev['ts'];
					if (!isset($emittedTs[$prevTs])) {
						send_event($prev + [
							'type' => 'row',
							'processed' => $processed,
							'anomalies' => $anomalies,
						...
						$emittedTs[$prevTs] = true;
					}
				}
				$shouldEmit = true; // emit anomaly row itself
				$postEmitRemaining = max($postEmitRemaining, 10);
			} elseif ($postEmitRemaining > 0) {
				$shouldEmit = true; // emit following 10 rows after an anomaly
			} elseif (TEST_SAMPLE_EVERY_ROWS > 0 && ($processed % TEST_SAMPLE_EVERY_ROWS) === 0) {
				$shouldEmit = true; // periodic sampling
			}

			if ($shouldEmit && !isset($emittedTs[$ts])) {
				send_event($currentPayload + [
					'type' => 'row',
					'processed' => $processed,
					'anomalies' => $anomalies,
					'total' => $total,
				...
				$emittedTs[$ts] = true;
				if ($postEmitRemaining > 0) {
					$postEmitRemaining--;
				}
			}

			// update context buffer with current row
			$contextBuffer[] = $currentPayload;
			if (count($contextBuffer) > $bufferCapacity) {
...
function handle_analyze_datapoints_test(): void {
	// Provide deterministic fake data
	$rows = [
		['id' => 101, 'name' => 'dp.strom.zaehlerstand', 'anzahl' => 123456],
		['id' => 102, 'name' => 'dp.gas.zaehlerstand', 'anzahl' => 98765],
		['id' => 103, 'name' => 'dp.wasser.zaehlerstand', 'anzahl' => 54321],
	];
	json_response(['ok' => true, 'rows' => $rows]);
}
...
function handle_test_with_test_data(): void {
	...
	// Generate and stream test data (selective emission)
	$context = [];
	$postEmitRemaining = 0;
	for ($i = 0; $i < $total; $i++) {
		...
		$shouldEmit = false;
		if ($isAnomaly) {
			foreach ($context as $prev) {
				send_event($prev + ['type' => 'row']);
			}
			$context = [];
			$shouldEmit = true;
			$postEmitRemaining = max($postEmitRemaining, 10);
...
if ($useTestData) {
	...
	switch ($action) {
		case 'analyze':
			handle_analyze_test();
			break;
		case 'analyze_datapoints':
			handle_analyze_datapoints_test();
			break;
		case 'test':
