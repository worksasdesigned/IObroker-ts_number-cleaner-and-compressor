    ...
    <script>
        ...
        async function doAnalyze() {
            ...
            try {
                ...
                fd.set('id', String(Number($('#analyzeId').value || DEFAULT_ID)));
                const f = $('#analyzeFrom').value; const t = $('#analyzeTo').value;
        ...
        async function doTest() {
            ...
            try {
                const fd = new FormData();
                fd.set('action', 'test');
                fd.set('id', String(Number($('#testId').value || DEFAULT_ID)));
                fd.set('intervalKey', getSelectedIntervalKey());
                const f = $('#from').value; const t = $('#to').value; if (!f || !t) throw new Error('Bitte Von/Bis wählen');
        ...
        async function doRun() {
            ...
            try {
                const fd = new FormData();
                fd.set('action', 'run');
                fd.set('id', String(Number($('#runId').value || DEFAULT_ID)));
                fd.set('intervalKey', getSelectedIntervalKey());
                const f = $('#runFrom').value; const t = $('#runTo').value; if (f) fd.set('from', f); if (t) fd.set('to', t);
        ...
        window.addEventListener('DOMContentLoaded', function() {
            ...
            $('#from').value = fmt(fromDate); $('#to').value = fmt(toDate);
            const elAnalyzeFrom = $('#analyzeFrom'); if (elAnalyzeFrom) elAnalyzeFrom.value = fmt(fromDate);
            const elAnalyzeTo = $('#analyzeTo'); if (elAnalyzeTo) elAnalyzeTo.value = fmt(toDate);
        });
    </script>
        ...
        <section class="panel stack">
            ...
            <div class="actions">
                <div class="panel">
                    ...
                    <div class="form-row"><label>Von</label><input type="datetime-local" id="from" required></div>
                    <div class="form-row"><label>Bis</label><input type="datetime-local" id="to" required></div>
                    <div class="form-row"><label>ID</label><input type="number" id="testId" value="<?php echo (int) DEFAULT_STATE_ID; ?>" min="1"></div>
                ...
                <div class="panel">
                    <h2>2. ECHTLAUF</h2>
                    <div class="form-row"><label>ID</label><input type="number" id="runId" value="<?php echo (int) DEFAULT_STATE_ID; ?>" min="1"></div>
                    <div class="form-row"><label>Von (optional)</label><input type="datetime-local" id="runFrom"></div>
                    <div class="form-row"><label>Bis (optional)</label><input type="datetime-local" id="runTo"></div>
                    <div class="form-row"><label>Intervall</label>
                        <div class="intervals">
                            <?php foreach (INTERVALS as $k=>$ms): ?>
                                <label><input type="radio" name="intervalKey" value="<?php echo htmlspecialchars($k, ENT_QUOTES); ?>" <?php echo $k==='1h'?'checked':''; ?>> <?php echo htmlspecialchars($k); ?></label>
                ...
                <div class="panel">
                    ...
                    <div class="form-row"><label>ID</label><input type="number" id="analyzeId" value="<?php echo (int) DEFAULT_STATE_ID; ?>" min="1"></div>
                    <div class="form-row"><label>Von (optional)</label><input type="datetime-local" id="analyzeFrom"></div>
                    <div class="form-row"><label>Bis (optional)</label><input type="datetime-local" id="analyzeTo"></div>
                    <div style="margin-top:10px;display:flex;gap:10px;">
                        <button class="btn btn-outline" id="btnAnalyze" onclick="doAnalyze()">Analyse starten</button>
                    </div>
